...
class Trainer:
    ...
    def __init__(self, config, data_loader):
        ...
        self.std = config.std
        self.M = config.M
        self.timeString = get_timeString()
        ...
        self.num_channels = 3 if config.dataset == "Cifar10" else 1
        ...
        with tqdm(total=self.num_train) as pbar:
            for i, (x, y) in enumerate(self.train_loader):
                ...
                for t in range(self.num_glimpses - 1):
                    ...
                    h_t, l_t, b_t, p, c_t = self.model(x, l_t, h_t, c_t)
                ...
                # last iteration
                h_t, l_t, b_t, log_probas, p, c_t = self.model(x, l_t, h_t, c_t, last=True)
                log_pi.append(p)
                baselines.append(b_t)
                locs.append(l_t[0:9])
    ...
    def test(self):
        ...
        for i, (x, y) in enumerate(self.test_loader):
            ...
            ########### shift test ###########
            batsh_s, channels, width, height = x.shape
            shift_unit = 1
            s_u = shift_unit * 2
            ...
            copy_tensor[:, :, ver_pos: ver_pos+width, her_pos: her_pos+height] = x
            x = copy_tensor[:, :, init_pos: init_pos+width, init_pos: init_pos+height]
            ########### shift test ###########
        ...
        # max_value = 0
        # max_value_index = 0 
        max_value = max(self.valid_acc_list)
        max_value_index = self.valid_acc_list.index(max_value)

        str_perc = str(round(perc.item(), 2))
        max_value = str(round(max_value, 2))

        if len(str_perc) == 4: str_perc += '0'
        if len(max_value) == 4: max_value += '0'

        self.record_path = f"../../hunternet/records/RAM/{self.dataset}/{self.memory_type}_{self.patch_size}x{self.patch_size}_{self.num_glimpses}.txt"
        info = f'Time: {self.timeString} | Val max acc: {max_value} at {max_value_index} | Test acc: {str_perc} | Random seed: {self.random_seed}'


        with open(self.record_path, "a+") as outfile:
