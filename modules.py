...
class CoreNetwork(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, memory_type):
        super().__init__()

        # self.memory_type = 'rnn'
        self.memory_type = memory_type

        if self.memory_type == 'o_rnn':
        ...
        elif self.memory_type == 'rnn':
            self.memory = nn.RNN(hidden_size, hidden_size, 1, nonlinearity='relu', batch_first=False)

        elif self.memory_type == 'lstm':
            self.memory = nn.LSTM(hidden_size, hidden_size, 1, batch_first=False)
            
        elif self.memory_type == 'gru':
            self.memory = nn.GRU(hidden_size, hidden_size, 1,  batch_first=False)

    def forward(self, g_t, h_t_prev, c_t):
        ...
        elif self.memory_type == 'rnn' or self.memory_type == 'gru':
            h_t, _ = self.memory(g_t.unsqueeze(0), h_t_prev.unsqueeze(0))
            h_t = h_t.squeeze(0)
            c_t = None

        elif self.memory_type == 'lstm':
            _, (h_t, c_t) = self.memory(g_t.unsqueeze(0), (h_t_prev.unsqueeze(0), c_t.unsqueeze(0)))
